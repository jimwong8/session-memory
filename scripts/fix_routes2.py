#!/usr/bin/env python3
"""Fix MissingGreenlet errors - use SessionResponse + manual assembly."""
with open("/app/src/routes.py", "r") as f:
    code = f.read()

# Fix: get_session route
...
new_get = """    svc = SessionService(db)
    ...
    if not session:
        raise HTTPException(status_code=404, detail="会话不存在")
    # Build from SessionResponse (no relationship access) + loaded data
    base = SessionResponse.model_validate(session)
...
if old_get in code:
    ...
    print("OK: get_session route fixed (use SessionResponse + manual build)")
else:
    print("WARN: get_session not found, trying original pattern")

# Fix: export_session route
