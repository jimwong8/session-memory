#!/usr/bin/env python3
"""AtlasCloud KG Worker - with debug"""
import time, json, re, uuid, requests, psycopg2
from psycopg2.extras import RealDictCursor
from requests.adapters import HTTPAdapter
from urllib3.util.retry import Retry

DB = "host=127.0.0.1 port=5432 dbname=session_memory user=postgres password=postgres"
...
def extract(text):
    prompt = f"Extract KG JSON: entities:[name,type],relations:[source,target,type]. Types: person,product,technology,concept,file,function,error,tool. Relations: depends_on,modifies,fixes,uses,created_by,leads_to. Text: {text[:1500]}"
    try:
        r = session.post(f"{AC_URL}/chat/completions", json={
            "model": MODEL, "messages": [{"role": "user", "content": prompt}],
            "max_tokens": 1000, "temperature": 0
        }, headers={"Authorization": f"Bearer {AC_KEY}"}, timeout=120)
        ...
        print(f"  err: {str(e)[:40]}", flush=True)
    return None
...
def main():
    ...
    print(f"[P{args.partition}] Connecting DB...", flush=True)
    ...
    while True:
        ...
        processed += 1
        if processed % 10 == 0:
            print(f"  [P{args.partition}] {processed} msgs", flush=True)
    
    cur.close()
    conn.close()
    print(f"  [P{args.partition}] DONE: {processed} msgs", flush=True)
