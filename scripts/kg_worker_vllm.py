...
BATCH = int(os.getenv("KG_BATCH", "200"))
MAX_WORKERS = int(os.getenv("KG_WORKERS", "32"))
# Hash partition support: each worker handles (hashtext(id) % TOTAL == PARTITION)
# so multiple worker lines (local CPU / cloud API) don't compete for the same jobs.
...
if __name__ == "__main__":
    # Continuous loop: keep processing batches without exiting, so the
    # 10s systemd RestartSec gap between one-shot runs is eliminated.
    while True:
        try:
            main()
        except Exception as e:
