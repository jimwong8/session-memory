...
# -*- coding: utf-8 -*-
"""
KG Backfill - Synchronous version using psycopg2 directly to avoid async issues.
"""
import argparse
import os
import sys
...
# Set up Django-style settings
os.environ.setdefault('DATABASE_URL', 'postgresql://postgres:postgres@postgres:5432/session_memory')

DB_URL = os.getenv('DATABASE_URL', 'postgresql://postgres:postgres@postgres:5432/session_memory')
...
def process_batch(conn, batch_size, sleep_seconds):
    ...
    with conn.cursor() as cur:
        ...
        for msg in messages:
            ...
            # For now, just mark as done - actual KG extraction needs async
            # We'll do a simple sync extract using psycopg2 directly
            ...
            # (Real extraction would need LLM - skip for sync backfill)
            ok = False
            ...
            if attempts >= 3:
                ...
                metadata['kg_extract_error'] = 'max attempts reached in sync backfill'
                ...
                print(f"deadletter={msg_id}")
                failed += 1
            else:
                # Mark as retry_wait for async processing later
...
def main():
    ...
    try:
        ...
        while True:
            p, s, f = process_batch(conn, args.batch_size, args.sleep_seconds)
            if p == 0:
