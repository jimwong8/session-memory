#!/usr/bin/env python3
"""KG worker - OpenCode Zen deepseek-v4-flash-free"""
...
MAX_WORKERS = int(os.getenv("KG_WORKERS", "15"))

def get_api_key():
    """获取 API key：优先 env，其次文件"""
    key = os.getenv("CUSTOM_PROVIDER_OPENCODE_GO_KEY", "")
    if key:
        return key
    try:
        with open("/tmp/zen_key.txt") as f:
            return f.read().strip()
    except:
        pass
    return ""
...
API_KEY = get_api_key()
if not API_KEY:
    print("WARNING: Zen API key not found", flush=True)
...
def extract(content):
    ...
    for attempt in range(3):
        try:
            ...
            if "429" in str(e) or "RateLimit" in type(e).__name__ or "Rate limit" in str(e):
...
def verify_entity_in_text(name, text):
    return name.lower() in text.lower() if name and text else False
    ...
    try:
        ...
        if result is None:
            cur.execute("UPDATE kg_jobs SET status='failed', last_error=%s, updated_at=now() WHERE id=%s",
                        ("Zen deepseek extract failed", job["id"]))
            conn.commit()
            return False
        ...
        for e in result.get("entities", []):
            ...
            try:
                ...
                conf = 1.0
            if conf < CONFIDENCE_THRESHOLD or not verify_entity_in_text(name, job["content"]):
                continue
...
def main():
    ...
    print(f"pending={pending} batch={len(jobs)} model={MODEL} key={'YES' if API_KEY else 'NO'}", flush=True)
    ok = fail = 0
