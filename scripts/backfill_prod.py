...
Uses CAST() to avoid SQLAlchemy :param escape issues with pgvector :: cast syntax.
"""
...
async def main():
    ...
    while True:
        async with async_session() as s:
            rows = (await s.execute(SELECT_SQL.bindparams(lim=BATCH))).all()
            if not rows:
                break
        ...
        async with async_session() as s:
            for mid, emb in zip(ids, embs):
                emb_str = str(emb)
                await s.execute(UPDATE_SQL.bindparams(emb=emb_str, id=mid))
            await s.commit()
        total += len(rows)
